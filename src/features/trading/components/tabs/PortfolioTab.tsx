...
const PortfolioTab: React.FC<PortfolioTabProps> = ({
  ...
  const totalPL = portfolioItems.reduce((sum, item) => sum + item.profitLoss, 0);
  const totalPLPercent = totalValue > 0 
    ? (portfolioItems.reduce((sum, item) => sum + item.profitLossPercent * item.marketValue, 0) / totalValue) || 0
    : 0;
            ...
            <div>
              ...
              <p className="text-sm mt-1">
                <span className={totalPL >= 0 ? 'text-green-600' : 'text-red-600'}>
                  {totalPL >= 0 ? '+' : ''}
                  ${Math.abs(totalPL).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th scope="col" className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Avg. Cost</th>
                  ...
                  {portfolioItems.map((item) => (
                    ...
                    >
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-sm text-gray-500">${item.currentPrice.toFixed(2)}</div>
                      </td>
                      ...
                      <td className={`px-6 py-4 whitespace-nowrap text-right ${item.profitLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
