...
class AIMusicEngine {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn('⚠️ OpenAI API key not configured. AI features will use fallback logic.');
    }
  }
   ...
   */
  async generateIntelligentPlaylist(request: AIPlaylistRequest): Promise<AIRecommendation> {
          ...
          functions: [
            {
              ...
              description: 'Create a DJ playlist with track recommendations and mixing advice',
              parameters: {
                type: 'object',
                properties: {
                  tracks: {
  ...
  async getNextTrackRecommendations(
    ...
    return recommendation.nextTrackSuggestions || recommendation.tracks.slice(0, 5);
  }
  ...
  async simulateCrowdResponse(
    ...
    try {
      ...
      const trackHistory = previousTracks.slice(-5).map(t => `${t.title} by ${t.artist} (${t.genre})`).join(', ');
      
      const prompt = `As a professional DJ reading crowd energy at a ${venue}, analyze the crowd response to playing "${currentTrack.title}" by ${currentTrack.artist} (${currentTrack.genre}, ${currentTrack.bpm} BPM, ${currentTrack.key} key) after recently playing: ${trackHistory}`;
      ...
      const response = await fetch(`${this.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: 'system',
              content: 'You are an expert DJ who can read crowd energy. Provide realistic crowd response analysis.'
            },
...
5. Provide practical mixing advice`;
  ...
  private mapAIResultToTracks(aiResult: any): AIRecommendation {
    ...
    while (tracks.length < 8 && trackPool.length > 0) {
      const randomTrack = trackPool.splice(Math.floor(Math.random() * trackPool.length), 1)[0];
      tracks.push(randomTrack);
    }

    return {
      tracks,
      reasoning: aiResult.reasoning || 'Curated playlist based on your preferences',
  ...
  private findBestTrackMatch(suggestion: any, trackPool: Track[]): Track | null {
    // Score tracks based on similarity to AI suggestion
    const scores = trackPool.map(track => {
      let score = 0;
      
      // Genre match
      if (track.genre.toLowerCase().includes(suggestion.genre?.toLowerCase() || '')) score += 30;
      
      // BPM proximity
      const bpmDiff = Math.abs(track.bpm - (suggestion.bpm || 120));
      score += Math.max(0, 20 - bpmDiff);
      
      // Energy match
      const energyMap = { low: 1, medium: 2, high: 3 };
      const trackEnergy = track.energy === 'low' ? 1 : track.energy === 'medium' ? 2 : 3;
      const suggestionEnergy = energyMap[suggestion.energy as keyof typeof energyMap] || 2;
  ...
  private generateDefaultMixingTips(tracks: Track[]): string[] {
    ...
    for (let i = 0; i < tracks.length - 1; i++) {
      ...
      if (Math.abs(bpmDiff) <= 3) {
        tips.push(`Smooth BPM transition from ${current.title} to ${next.title} (${bpmDiff > 0 ? '+' : ''}${bpmDiff} BPM)`);
      } else {
        tips.push(`Use tempo adjustment for ${current.title} → ${next.title} transition (${bpmDiff > 0 ? 'speed up' : 'slow down'})`);
      }
    }
  ...
  private generateFallbackPlaylist(request: AIPlaylistRequest): AIRecommendation {
    ...
    if (request.bpmRange) {
      tracks = tracks.filter(t => 
        t.bpm >= request.bpmRange!.min && t.bpm <= request.bpmRange!.max
      );
    }
    
    // Sort by mood preference
    if (request.mood === 'energetic') {
      tracks.sort((a, b) => (b.bpm + (b.energy === 'high' ? 20 : 0)) - (a.bpm + (a.energy === 'high' ? 20 : 0)));
    } else if (request.mood === 'chill') {
      tracks = tracks.filter(t => t.energy === 'low' || t.energy === 'medium');
    ...
    return {
      tracks,
      reasoning: 'Curated playlist based on your preferences using intelligent fallback selection',
  ...
  private analyzeMoodFallback(input: string): MoodAnalysis {
    ...
    let genre = 'Electronic', bpm = 125;
    
    // Simple keyword analysis
    if (inputLower.includes('energetic') || inputLower.includes('party') || inputLower.includes('dance')) {
      energy = 80; danceability = 85;
    }
    if (inputLower.includes('chill') || inputLower.includes('relax') || inputLower.includes('ambient')) {
      energy = 30; bpm = 110;
    }
    ...
    return {
      energy, valence, danceability, dominantGenre: genre, suggestedBPM: bpm,
      description: `Analyzed mood: ${energy > 70 ? 'High energy' : energy > 40 ? 'Medium energy' : 'Low energy'} ${genre.toLowerCase()} music`
    };
  }

  private simulateCrowdResponseFallback(track: Track, venue: string): {
    energy: number;
    response: string;
    suggestions: string[];
  } {
    const baseEnergy = track.energy === 'high' ? 75 : track.energy === 'medium' ? 50 : 30;
    const venueModifier = venue === 'club' ? 15 : venue === 'festival' ? 20 : 0;
    const energy = Math.min(100, baseEnergy + venueModifier + Math.random() * 20);
    
    return {
      energy,
      response: energy > 70 ? 'Crowd is loving it! High energy on the dance floor.' : 
               energy > 40 ? 'Good response, people are feeling the vibe.' : 
               'Mellow response, perfect for building atmosphere.',
      suggestions: energy > 70 ? 
  ...
  private getAvailableGenres(): string[] {
    return [...new Set(MUSIC_LIBRARY.map(t => t.genre))];
  }
