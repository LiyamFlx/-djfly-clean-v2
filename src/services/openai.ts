...
<<<<<<< HEAD
  ...
  async analyzeCrowdAudio(
    ...
    try {
      ...
      return this.parseAudioAnalysisResponse(content);
    } catch (error) {
      ...
      try {
        ...
        if (response.status === 429) {
          ...
          if (attempt < maxRetries) {
            console.warn(`⏳ OpenAI rate limit hit, retrying in ${waitTime}ms (attempt ${attempt + 1}/${maxRetries + 1})`);
        ...
        if (attempt < maxRetries) {
          ...
          console.warn(`🔄 OpenAI request failed, retrying in ${waitTime}ms:`, error);
          await new Promise(resolve => setTimeout(resolve, waitTime));
        }
      }
    }
...
<<<<<<< HEAD
      const systemPrompt = `You are an expert DJ and music curator. Generate a detailed playlist recommendation based on the user's request. Consider energy levels, mood, venue type, and crowd preferences. Return a JSON response with the following structure:
      {
        "tracks": [
          {
            "title": "Track Title",
