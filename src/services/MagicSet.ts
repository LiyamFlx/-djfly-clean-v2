...
export class MagicSet {
  ...
  // Set Building
  async buildSet(context: SessionContext, availableTracks: Track[]): Promise<MagicSetResult> {
  ...
  private async generateInitialSet(context: SessionContext): Promise<{ tracks_order: string[], energy_curve: EnergyPoint[] }> {
    ...
    let filteredTracks = this.tracks.filter(track => {
      ...
      if (genres && genres.length > 0 && track.genre) {
        if (!genres.some(genre => track.genre!.toLowerCase().includes(genre.toLowerCase()))) {
          return false;
        }
      }
  ...
  // Track Matching
  async findMatches(baseTrackId: string, limit: number = 10): Promise<TrackMatch[]> {
  ...
  private async calculateTrackMatch(baseTrack: Track, candidateTrack: Track): Promise<TrackMatch> {
    const harmonicCompatibility = this.calculateHarmonicCompatibility(baseTrack, candidateTrack);
    ...
    const transitionQuality = this.calculateTransitionQuality(baseTrack, candidateTrack);
    
    const score = (harmonicCompatibility * 0.3) + 
                  (energyFlow * 0.3) + 
                  (crowdAppeal * 0.2) + 
  ...
  insertTrack(trackId: string, position: number): void {
    ...
    this.setState.energyCurve = this.recalculateEnergyCurve(this.setState.order);
  }
  ...
  private calculateSetScore(order: string[], energyCurve: EnergyPoint[]): number {
    let score = 0;
    
    // Energy flow score
    for (let i = 1; i < energyCurve.length; i++) {
      const energyDiff = Math.abs(energyCurve[i].value - energyCurve[i-1].value);
      score += Math.max(0, 1 - energyDiff); // Prefer smooth transitions
    }
    
    // Track variety score
    const uniqueArtists = new Set(order.map(id => this.tracks.find(t => t.id === id)?.artist)).size;
  ...
  private calculateConfidence(track1: Track, track2: Track): number {
    ...
    const hasAudioFeatures = track1.bpm && track1.key && track2.bpm && track2.key;
  ...
  private generateOptimizationRationale(order: string[], energyCurve: EnergyPoint[]): string {
    const trackCount = order.length;
    const avgEnergy = energyCurve.reduce((sum, point) => sum + point.value, 0) / energyCurve.length;
    const energyVariance = energyCurve.reduce((sum, point) => sum + Math.pow(point.value - avgEnergy, 2), 0) / energyCurve.length;
