...
export const testApiConnections = async () => {
  ...
  try {
    ...
    console.log(
      `Supabase: ${results.supabase ? '✅' : '❌'} ${response.status}`
    );
  } catch {
  ...
  try {
    const response = await fetch(
      `https://www.googleapis.com/youtube/v3/search?part=snippet&q=test&key=${secureConfig.youtube.apiKey}`
    );
  ...
  try {
    const response = await fetch(
      `https://ws.audioscrobbler.com/2.0/?method=track.search&track=test&api_key=${secureConfig.lastfm?.apiKey}&format=json`
    );
  ...
  try {
    const response = await fetch(
      `https://www.googleapis.com/analytics/v3/management/accounts?key=${secureConfig.googleStudio?.apiKey}`
    );
    results.googleStudio = response.status !== 401;
    console.log(
      `Google Studio: ${results.googleStudio ? '✅' : '❌'} ${response.status}`
    );
  } catch {
