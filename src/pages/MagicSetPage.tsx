...
const MagicSetPage = () => {
  ...
  const detectMoodFromPrompt = (prompt: string): 'energetic' | 'chill' | 'progressive' | 'mixed' => {
    ...
    if (lowerPrompt.includes('energetic') || lowerPrompt.includes('party')) return 'energetic';
    if (lowerPrompt.includes('chill') || lowerPrompt.includes('lounge')) return 'chill';
      ...
      {status === 'complete' && (
        <div>
            <div className="bg-green-900 bg-opacity-30 p-6 rounded-lg mb-6 text-center">
                <h2 className="text-2xl font-bold mb-2">✅ AI Playlist Generated!</h2>
                {aiRecommendation && <p className="text-sm text-green-300 mb-4">🤖 {aiRecommendation.reasoning}</p>}
                <div className="flex gap-4 justify-center">
                    <button onClick={() => navigate('/player')} className="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-6 rounded-full">▶ Play Now</button>
                    <button onClick={resetGenerator} className="bg-gray-600 hover:bg-gray-700 text-white font-bold py-2 px-6 rounded-full">🔄 Create Another</button>
                </div>
            </div>
