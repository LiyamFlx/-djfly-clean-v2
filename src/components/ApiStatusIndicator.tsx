...
import { runAllHealthChecks, HealthCheckResult, ServiceName, ServiceStatus } from '@/utils/apiHealth';
            ...
            >
              <div className="p-4 space-y-3">
                <div className="space-y-2">
                  <ServiceStatusRow result={services.spotify} name="Spotify" description="Music data & tracks" />
                  <ServiceStatusRow result={services.supabase} name="Supabase" description="Database & storage" />
                  <ServiceStatusRow result={services.openai} name="OpenAI" description="AI recommendations" />
                </div>
                <button
                ...
                >
                  {isTesting ? (
                    <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div><span>Testing...</span></>
                  ) : (
                    <><span>🔄</span><span>Test Connections</span></>
                  )}
                </button>
              </div>
            </motion.div>
...
const ServiceStatusRow: React.FC<{
  ...
  const message = result?.message || 'Not tested';
    ...
    <div className="flex items-center justify-between">
      <div className="flex-1">
        <div className="flex items-center space-x-2">
          <span className={`text-sm font-medium ${status !== 'disconnected' ? 'text-white' : 'text-gray-400'}`}>
            {name}
          </span>
          <span className="text-xs">{status === 'connected' ? '✅' : status === 'demo' ? '🔵' : '❌'}</span>
        </div>
        <div className="text-xs text-gray-500">{description}</div>
      </div>
      <div className={`px-2 py-1 rounded text-xs font-medium capitalize ${statusStyles[status]}`}>
        {status}
      </div>
    </div>
  );
