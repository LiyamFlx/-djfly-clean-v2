...
export const validateApiConfig = () => {
  ...
  const isPlaceholderSpotifyId = API_CONFIG.spotify.clientId?.match(/^(abc123|e5050e55f5a94ca2)/);
  const isPlaceholderSpotifySecret = API_CONFIG.spotify.clientSecret?.match(/^(xyz456|7e76b6a7c1434b1a)/);
  
  const validations = {
    spotify: {
      clientId: !!API_CONFIG.spotify.clientId && API_CONFIG.spotify.clientId.length > 10 && !isPlaceholderSpotifyId,
      clientSecret: !!API_CONFIG.spotify.clientSecret && API_CONFIG.spotify.clientSecret.length > 10 && !isPlaceholderSpotifySecret,
      redirectUri: !!API_CONFIG.spotify.redirectUri,
      isPlaceholder: !!(isPlaceholderSpotifyId || isPlaceholderSpotifySecret),
    },
    supabase: {
      url: !!API_CONFIG.supabase.url && API_CONFIG.supabase.url.startsWith('https://'),
      anonKey: !!API_CONFIG.supabase.anonKey && API_CONFIG.supabase.anonKey.startsWith('eyJ'),
    },
    openai: {
      apiKey: !API_CONFIG.openai.apiKey || (API_CONFIG.openai.apiKey.startsWith('sk-') && API_CONFIG.openai.apiKey.length > 40),
    },
    youtube: {
      apiKey: !!API_CONFIG.youtube.apiKey && API_CONFIG.youtube.apiKey.length > 20,
    },
    lastfm: {
      apiKey: !!API_CONFIG.lastfm.apiKey && API_CONFIG.lastfm.apiKey.length > 20,
      secret: !!API_CONFIG.lastfm.secret && API_CONFIG.lastfm.secret.length > 20,
    },
    googleStudio: {
      apiKey: !!API_CONFIG.googleStudio.apiKey && API_CONFIG.googleStudio.apiKey.length > 20,
    }
  };
  ...
  Object.entries(validations).forEach(([service, checks]) => {
    ...
    console.log(`${service.charAt(0).toUpperCase() + service.slice(1)}: ${allValid ? '✅' : '❌'}`);
    if (!allValid) {
      Object.entries(checks).forEach(([key, valid]) => {
        if (!valid) {
          if (key === 'isPlaceholder' && (checks as any).isPlaceholder) {
            console.warn(`  🔄 ${service}: Using placeholder/demo credentials - see API_CREDENTIALS_GUIDE.md`);
          } else {
